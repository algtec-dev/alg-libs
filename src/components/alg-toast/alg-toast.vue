  ...
  <div>
    ...
    >
      ...
      <template v-slot:action>
        ...
        >
          <v-btn icon @click="removeMessage(snackbar.key, true)"
...
export default {
  ...
  methods: {
    showSuccessToast(text) {
      this.objects.push({ message: text, color: "green" });
    },
    showErrorToast(text) {
      this.objects.push({ message: text, color: "red" });
    },
    getProp(prop, i) {
      if (
    ...
    setSnackbars() {
      for (let i = this.snackbars.length; i < this.allMessages.length; i++) {
        ...
        this.snackbars.push({
          ...
          contentClass:
            this.getProp("content-class", i) ||
            this.getProp("contentClass", i) ||
            "",
          timeout: null,
          transition:
